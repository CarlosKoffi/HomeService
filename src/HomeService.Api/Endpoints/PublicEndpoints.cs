...
public static class PublicEndpoints
{
    public static IEndpointRouteBuilder MapPublicEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapGet("/api/cms/client/home", async (
            string? language,
            string? country,
            ClientHomeCmsQueryService queryService,
            CancellationToken cancellationToken) =>
        {
            var response = await queryService.GetAsync(language, country, cancellationToken);
            return response is null
                ? Results.NotFound(new { message = "Contenu CMS client introuvable." })
                : Results.Ok(response);
        })
        .WithName("GetClientHomeCmsContent")
        .Produces<ClientHomeCmsResponse>()
        .Produces(StatusCodes.Status404NotFound);
        ...
        client.MapPut("/payment-methods/mobile-money/{paymentMethodId:guid}", async (
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            var result = await profileService.UpdateMobileMoneyAccountAsync(
                customer.Id,
                paymentMethodId,
                request,
                cancellationToken);
        ...
        .Produces<CreateClientMobileMoneyAccountResponse>()
        .Produces(StatusCodes.Status400BadRequest)
        .Produces(StatusCodes.Status401Unauthorized);
