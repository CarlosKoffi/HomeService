...
public sealed class PlatformApiClient(HttpClient httpClient, IConfiguration configuration)
{
    ...
    public async Task<EmployeeSaveResult> MarkCompanyNotificationReadAsync(
        Guid companyId,
        Guid notificationId,
        CancellationToken cancellationToken = default)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var response = await httpClient.PostAsJsonAsync(
            ...
            new CompanyReviewInterimCandidateRequest(
                note,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var response = await httpClient.PostAsJsonAsync(
            ...
            new CompanyReviewInterimCandidateRequest(note, false, false, false, false),
            cancellationToken);
        var body = await response.Content.ReadAsStringAsync(cancellationToken);
        return response.IsSuccessStatusCode
            ? new EmployeeSaveResult(true, null)
    ...
    {
        AddBasicAuthIfConfigured();
        var response = await httpClient.PostAsJsonAsync($"/api/company-portal/{companyId:D}/employees/{employeeId:D}/availability", request, cancellationToken);
        var body = await response.Content.ReadAsStringAsync(cancellationToken);
        return response.IsSuccessStatusCode
            ? new EmployeeSaveResult(true, null)
            : new EmployeeSaveResult(false, ExtractErrorMessage(body) ?? response.ReasonPhrase ?? "Action impossible.");
