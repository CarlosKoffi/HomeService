...
public sealed class ClientMissionConfirmationServiceTests
{
    ...
    public async Task ConfirmAsync_WhenProviderAcceptedMission_AuthorizesPaymentAndReleasesContacts()
    {
        ...
        Assert.Equal(800, result.Response.CustomerServiceFeeAmount);
        Assert.Equal(400, result.Response.CustomerServiceFeeRateBasisPoints);
        Assert.Equal(20_800, result.Response.TotalAmount);
        ...
        Assert.Equal(20_800, (await db.MissionPaymentMilestones.SingleAsync()).Amount);
