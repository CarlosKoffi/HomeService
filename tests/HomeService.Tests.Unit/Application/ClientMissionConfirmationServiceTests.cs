...
public sealed class ClientMissionConfirmationServiceTests
{
    ...
    public async Task ConfirmAsync_WhenProviderAcceptedMission_AuthorizesPaymentAndReleasesContacts()
    {
        ...
        Assert.Equal(20_000, result.Response.ServiceAndPartsAmount);
        Assert.Equal(1_500, result.Response.CustomerServiceFeeAmount);
        Assert.Equal(750, result.Response.CustomerServiceFeeRateBasisPoints);
        Assert.Equal(21_500, result.Response.TotalAmount);
        Assert.Equal(3_000, scenario.Mission.PlatformCommissionAmount);
        Assert.Equal(17_000, scenario.Mission.CompanyPayoutAmount);
        ...
        Assert.Equal(21_500, (await db.MissionPaymentMilestones.SingleAsync()).Amount);
