...
{
    [Fact]
    public async Task CalculateAsync_FirstOrder_ExcludesPartsAndAppliesFirstOrderRate()
    {
        ...
        Assert.Equal(3_000, pricing.CompanyCommissionAmount);
        Assert.Equal(1_000, pricing.CustomerServiceFeeAmount);
        Assert.Equal(31_000, pricing.CustomerTotalAmount);
        Assert.Equal(27_000, pricing.CompanyPayoutAmount);
    }

    [Fact]
    public async Task CalculateAsync_RepeatOrder_AppliesReducedCompanyRate()
    {
        await using var db = CreateDbContext();
        ...
        Assert.Equal(900, pricing.CompanyCommissionRateBasisPoints);
        Assert.Equal(2_250, pricing.CompanyCommissionAmount);
        Assert.Equal(27_750, pricing.CompanyPayoutAmount);
        Assert.Equal(31_000, pricing.CustomerTotalAmount);
    }
